...
(function () {
  ...
  whenExists('#planos', function (PLANS_SECTION) {
    if (!PLANS_SECTION) {
      // não encontrou #planos dentro do tempo; aborta inicialização silenciosamente
      return;
    }

    // cria painel reutilizável
    ...
    let loadedCases = {}; // cache por caminho
    let currentPath = null;
    let lastTrigger = null;
    let inflight = {}; // evita múltiplas requests idênticas simultâneas
    ...
    // utilidade: inicializa carrosséis simples encontrados no conteúdo do case
    function initCaseCarousels(container) {
      ...
      carousels.forEach((wrap) => {
        ...
        // show first slide and start autoplay if enabled
        show(0);
        startTimer();
      });
    }
    ...
    document.addEventListener('click', function (e) {
      ...
      fetchCase(path).then(function (html) {
        ...
        // Alternativa escolhida: criar clone do heading para o cabeçalho e esconder o heading original
        const heading = CASE_CONTENT.querySelector('h2, h1, h3, h4');
        if (heading) {
          CASE_HEADING_WRAPPER.innerHTML = '';

          // cria clone acessível para o cabeçalho do painel
          const clone = heading.cloneNode(true);
          clone.classList.add('text-xl', 'font-extrabold', 'text-secondary');
          clone.setAttribute('aria-hidden', 'false');
          CASE_HEADING_WRAPPER.appendChild(clone);

          // esconder o heading original para evitar duplicação visual e leitura duplicada por leitores de tela
        ...
        // caso raro: erro não tratado pelo fetchCase
