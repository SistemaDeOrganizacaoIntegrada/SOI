...
(function () {
  ...
  // unified routine to apply both JS state classes and BEM modifiers
  function applySlideState(s, active) {
    // JS-facing state
    s.classList.toggle('is-visible', active);
    s.classList.toggle('is-active', active);

    // BEM modifiers expected by some CSS files
    s.classList.toggle('hero-slide--visible', active);
    s.classList.toggle('hero-slide--active', active);

    // carousel specific modifier
    if (s.classList.contains('carousel-slide')) {
      s.classList.toggle('carousel-slide--active', active);
    }

    // ensure ARIA consistency
  ...
  // normalize images inside a slide node: transfer data-src -> src, add responsive classes, keep lazy by default
  function normalizeSlideImages(node, { makeEager = false } = {}) {
    try {
      const imgs = node.querySelectorAll('img');
      imgs.forEach(img => {
        const real = img.dataset?.src || img.getAttribute('data-src') || img.getAttribute('src') || img.src;
        if (real && img.getAttribute('src') !== real) img.setAttribute('src', real);
        // default to lazy for performance; initial slide may be forced to eager
        if (!img.getAttribute('loading')) img.setAttribute('loading', makeEager ? 'eager' : 'lazy');
        if (!img.getAttribute('decoding')) img.setAttribute('decoding', 'async');
        img.classList.add('w-full', 'max-w-md', 'rounded-2xl', 'object-cover');
      });
    } catch (e) { /* silent */ }
  }
  ...
  async function init() {
    ...
    if (window.SOIHero && SOIHero.Slides && typeof SOIHero.Slides.buildSlides === 'function') {
      ...
      // normalize imgs in slides created by builder
      slides.forEach(s => normalizeSlideImages(s, { makeEager: false }));
    } else {
      ...
      slides = fragments.map(html => {
        ...
        container.className = 'hero-state hero-slide';
        container.setAttribute('aria-hidden', 'true');

        // preserve headings/paragraphs as before, but also preserve images
        // append all child nodes to keep markup (safer than cloning only heading/para)
        Array.from(temp.childNodes).forEach(n => container.appendChild(n.cloneNode(true)));

        // normalize images inside this fallback node
        normalizeSlideImages(container, { makeEager: false });
    ...
    try {
      ...
      if (initialSlide) {
        normalizeSlideImages(initialSlide, { makeEager: true });
      }
    } catch (e) { /* ignore */ }

    // apply initial state with BEM compatibility
    slides.forEach((s, idx) => {
      const active = idx === current;
      applySlideState(s, active);
    });
